...
strcat(filenameBuffer,"vacuum.log");
	...
	/*Asking the user for storing the results into a file */
	fprintf(stdout,"Do yo want to store the results into a file? (y/n): ");
	scanf("%c",&dec);
	getchar();
	...
	if (dec=='Y' || dec=='y'){
		/*writing file*/
		fprintf(stdout,"   Enter file name (prefereably with .tpc extension)\n");
...
/* flag <salir> is modified when signal SIGTERM is received */
	while (!salir){
		/*checking if the log files exists */
...
strcat(filenameBuffer,"medida.log");
		...
		/* Showing the menu and selecting the option */
		switch (menu(haybd, haylogs)){

			case 1: /* Creating the database */
				/* Asking for the number of warehouses */
				fprintf(stdout, "Enter the number of warehouses: ");	
				fscanf(stdin, "%d", &w);
				getchar();
				...
				ch = getchar();
				getchar();
				if ((ch != 'y') && (ch != 'Y')) break; /* exit case */

				/* Checking the number of warehouses entered */
				if ((w > NUM_MAX_ALM) || (w <= 0)){
					fprintf(stdout, "The number of warehouses must be between 1 and %d\n", NUM_MAX_ALM);
					break;
				}
		...
		case 2: /* Restoring the database */
			...
			ch = getchar();
			getchar();
			if ((ch != 'y')&&(ch != 'Y')) break; /* Salimos de case */
...
strcat(filenameBuffer,"cons.dat");
			...
			ch = 'n'; 
			do {
				fprintf(stdout, "->Enter number of warehouses ");	
				fscanf(stdin, "%d", &w);
				getchar();
				fprintf(stdout, "->Enter number of terminals per warehouse (max. 10) ");	
				fscanf(stdin, "%d", &term);
				getchar();
				fprintf(stdout, "->Enter ramp-up period (minutes) ");	
				fscanf(stdin, "%d", &int_ramp);
				getchar();
				int_ramp = int_ramp * 60;
				fprintf(stdout, "->Enter measurement interval (minutes) ");	
				fscanf(stdin, "%d", &int_med);
				getchar();
				int_med = int_med * 60;
				/* Checking entered data */
				if ((w <= 0) ||(w > NUM_MAX_ALM)||(int_ramp <= 0) || (term <= 0) || (term > 10) || (int_med <= 0)){
					fprintf(stdout,"Error:-> wrong parameters\n");
					ch = 'n';
				} else
					ch = 'y';
			}while (ch == 'n');
			...
			vacuum=0;
			fprintf(stdout,"->Do you want to perform vacuums during the test? (y/n): ");
			dec_vacuum=getchar();
			getchar();
			if (dec_vacuum=='Y' || dec_vacuum=='y'){
				do{
					fprintf(stdout,"	->Enter interval between vacuums (minutes): ");
					scanf("%d",&int_vacuum);
					getchar();
					if (int_vacuum<=0) fprintf(stdout,"		->Interval not valid.\n"); 
				}
				while(int_vacuum<=0);
				fprintf(stdout,"->Do you want to specify the maximum number of vacuums? (y/n): ");
				dec_n_vacuum=getchar();
				getchar();
				if (dec_n_vacuum=='Y' || dec_n_vacuum=='y'){
					do{
						fprintf(stdout,"	->Enter the maximum number of vacuums: ");
						scanf("%d",&num_vacuum);
						getchar();
					if (num_vacuum<0) fprintf(stdout,"		->Value not valid.\n"); 
					}while(num_vacuum<0);
				}else num_vacuum=0;
				fprintf(stdout,"->Performing vacuums every %d minutes",int_vacuum);
				if (dec_n_vacuum=='Y' || dec_n_vacuum=='y'){
					fprintf(stdout,", to reach a maximum number of %d.\n",num_vacuum);
				}else{
					fprintf(stdout,".\n");
				}				
			}else{
			...
			/* Asking for continue */
			fprintf(stdout, "Start the test? (y/n): ");
			ch = getchar();
			getchar();
			if ((ch != 'y')&&(ch != 'Y')) break; /* exit case */
...
strcat(filenameBuffer,"medida.log");
		...
		case 5: /*delete database*/
			...
			fprintf(stdout,"Delete database?: y/n ");
			ch = getchar();
			getchar();
			if ((ch != 'y')&&(ch != 'Y')){
				fprintf(stdout,"Database not modified.\n");
				break; /* exit case */
			}
